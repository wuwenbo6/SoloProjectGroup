require('dotenv').config({ path: '../../.env' });
const express = require('express');
const cors = require('cors');
const helmet = require('helmet');
const morgan = require('morgan');
const { createDbPool } = require('../../shared/utils/db');
const batchController = require('./controllers/batchController');
const batchRoutes = require('./routes/batchRoutes');

const app = express();
const PORT = process.env.BATCH_SERVICE_PORT || 3005;

app.use(helmet());
app.use(cors());
app.use(morgan('combined'));
app.use(express.json());
app.use(express.urlencoded({ extended: true }));

const pool = createDbPool({
  host: process.env.BATCH_DB_HOST,
  port: process.env.BATCH_DB_PORT,
  database: process.env.BATCH_DB_NAME,
  user: process.env.BATCH_DB_USER,
  password: process.env.BATCH_DB_PASSWORD
});

batchController.initModels(pool);

app.use('/api/batches', batchRoutes);

app.use((err, req, res, next) => {
  console.error(err.stack);
  res.status(500).json({
    success: false,
    message: '服务器内部错误',
    timestamp: new Date().toISOString()
  });
});

app.listen(PORT, () => {
  console.log(`批次管理服务运行在端口 ${PORT}`);
  console.log(`健康检查: http://localhost:${PORT}/api/batches/health`);
});

module.exports = app;
